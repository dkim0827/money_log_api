class Transaction < ApplicationRecord
    ...
    before_validation :set_description_for_savings
    ...
    def set_description_for_savings
        if trans_type == "SAVINGS" && description.blank?
            self.description = self.trans_type
        end
    end
    ...
    # check for valid transaction_type = ["INCOME", "SAVINGS", "LE", "NLE"]
    # INCOME = Income
    # LE = Living Expense
    # NLE = Non-Living Expense
    def is_trans_type_valid?
        valid_trans_type = ["INCOME", "SAVINGS", "LE", "NLE"]
    ...
    def is_trans_date_valid?
        ...
        if !(trans_date.methods.include? :strftime)
            self.errors.add(:base, "Date must be selected")
        else
            # transaction date
